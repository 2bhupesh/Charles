...
- **EF Core + SQLite**, one file-based database per service (`events.db`, `accounts.db`). `:memory:` is prohibited (connection-scoped lifetime). Tests use isolated per-test database files or shared-cache connections. **Connection pooling is disabled** (`Pooling=False`): EF Core registers user-defined functions per connection and Microsoft.Data.Sqlite unregisters them when returning one to the pool, which races with active statements under concurrent requests and throws SQLite error 5 (`unable to delete/modify user-function due to active statements`) — surfacing as a 500 on a valid request. Reproduced at roughly 1 in 1,600 concurrent same-`eventId` requests before the fix. Opening a local-file connection is cheap, so the pool buys little and costs correctness under load.
...
### 3.6 `GET /accounts/{accountId}/balance` — balance proxy

Added in Phase 3. The Account Service is internal (§1), so without this endpoint a client has **no way to read a balance at all**, and Requirement §6 ("balance queries must return a clear error when the Account Service is unreachable") would be unsatisfiable. The Gateway does not compute or cache balances — it proxies the read through the resilient client (§7).

| Code | Meaning | Body |
|---|---|---|
| `200 OK` | Balance retrieved | `{ "accountId": "acct-123", "balance": 125.50, "currency": "USD" }` (§4.2 verbatim) |
| `404 Not Found` | The account has no transactions — genuinely absent, not a downstream failure | ProblemDetails |
| `503 Service Unavailable` | Account Service unreachable or circuit open | ProblemDetails + `Retry-After: 5` |

The `503` **names the Account Service explicitly** ("Account Service is unreachable, so the balance cannot be retrieved. The Gateway's event endpoints are unaffected."). Returning `404` here would be a lie: it would tell a client its account does not exist when the truth is that the balance service is down.

Unlike §3.2/§3.3 this endpoint **cannot degrade gracefully** — there is no local data to fall back on. That asymmetry is the point of §7.1.

### 3.7 `GET /metrics` — Prometheus exposition format (bonus, §9)
...
All values live in `appsettings.json` under **`AccountService:Resilience`**, bound to `HttpStandardResilienceOptions`, so tests can trip the breaker quickly (e.g., min 2 calls, 1 retry) without waiting out production windows. Note the options validate: `MaxRetryAttempts` ≥ 1 (retries cannot be disabled outright), and `CircuitBreaker.SamplingDuration` ≥ 2 × `AttemptTimeout`.
...
| `GET /accounts/{id}/balance` | `503` naming the Account Service — the only endpoint with no local fallback (§3.6) |
...
| G10 | Transient failure then success | The retry lands; client sees `201` and never learns of the blip |
