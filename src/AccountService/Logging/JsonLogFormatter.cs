...
/// the sake of a log format - the same reasoning as AccountService.Domain.TransactionType.
/// </remarks>
public sealed class JsonLogFormatter : ConsoleFormatter, IDisposable
{
        ...
        TextWriter textWriter)
    {
        ...
        using (var writer = new Utf8JsonWriter(buffer))
        {
            ...
            // service: ASP.NET Core starts it from the traceparent the Gateway sent
            // (SPEC 8.1), so both services report the same traceId.
    ...
    /// <summary>
    /// Promotes the named holes of a structured message ("Applied {Type} of {Amount}...")
    /// to their own fields, so logs can be filtered by eventId rather than by substring.
    /// </summary>
